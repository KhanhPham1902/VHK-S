...
import '../api/response/GpsResponse10Bytes.dart';
import '../api/response/GpsResponse15Bytes.dart';
...
class GpsLogScreen extends StatefulWidget{
    ...
    final String imei;
    final String owner;
    const GpsLogScreen({Key? key, required this.ship, required this.imei, required this.owner}) : super(key: key);
...
class _GpsLogScreenState extends State<GpsLogScreen> {
    ...
    bool _isMapLocked = false; // Biến kiểm soát thao tác trên map
  ...
  void initState() {
        ...
        if(widget.imei != null && widget.imei.isNotEmpty) {
            handleListPayload(true, widget.imei, _startDateRequest!, _endDateRequest!);
        }
  }
  ...
  void dispose() {
                                                ...
                                                children: [
                                                    ...
                                                    ? Column(
                                                        children: [
                                                            ...
                                                            InkWell(
                                                                onTap: () async {
                                                                    await _support.selectDateTime(context, DateTime(2000), (pickedDateTime) {
                                                                        setState(() {
                                                                            _startDateShow = _support.formatTimeShow(pickedDateTime);
                                                                            _startDateRequest = _support.formatRequestTime(pickedDateTime);
                                                                        });
                                                                        debugPrint("Selected start date show: $_startDateShow");
                                                                        debugPrint("Selected start date request: $_startDateRequest");
                                                                    });
                                                                },
                                                                child: Container(
                                                                    width: double.infinity,
                                                                    ...
                                                                    decoration: BoxDecoration(
                                                                        border: Border.all(color: Colors.grey, width: 1.0), // Viền ngoài
                                                            ...
                                                            // Nut tim kiem
                                                            CupertinoButton(
                                                                padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 10.0), // Độ dày lề nút
                                                                color: Colors.blueAccent,
    ...
    void _toggleMapType() {
        setState(() {
            ...
            String mapType = _currentMapType == MapType.normal ? 'bản đồ thường' : 'vệ tinh';
            _support.showSnackbar(context, "Đang hiển thị ở chế độ $mapType", Colors.blueAccent);
        });
    }

    // Xu ly danh sach payload
    Future<void> handleListPayload(bool isLoading, String imei, String startTime, String endTime) async {
        if(isLoading){
            _isLoading = true;
        }
        final formatImei = _support.formatImei(imei);
        try {
            listGpsLog = await fetchListPayload(formatImei, startTime, endTime, LIMIT);
    ...
    Future<List<GpsLogInfo>> fetchListPayload(String imei, String startDate, String endDate, String limit) async {
        try {
            List<PayloadResponse>? payloadResponse = await _apiService.getListPayload(imei, startDate, endDate, limit);
            ...
            if (payloadResponse != null && payloadResponse.isNotEmpty) {
                ...
                for (PayloadResponse payloadData in payloadResponse) {
                    ...
                    if (gpsLogResponse != null) {
                        var gpsLogInfo = GpsLogInfo(gpsResponse: gpsLogResponse, byteCount: lengthData, sessionTime: time);
    ...
    Future<void> _setUpGpsMarker(List<GpsLogInfo> gpsLogs) async {
        int retryCount = 0;
        while (!_isMapReady && retryCount < 50) {  // Giới hạn tối đa 5 giây (50 * 100ms)
        ...
        // Load icon marker
        Uint8List startMarkerIcon = await _support.resizeImage('assets/start_locate.png', 80);
        Uint8List endMarkerIcon = await _support.resizeImage('assets/end_locate.png', 80);
        Uint8List defaultMarkerIcon = await _support.resizeImage('assets/dot.png', 40);
        ...
        setState(() {
            ...
            for (int i = 0; i < gpsLogs.length; i++) {
                ...
                final LatLng coordinates = LatLng(gpsLog.gpsResponse.latitude, gpsLog.gpsResponse.longitude);
                _polylineCoordinates.add(coordinates);

                // Xác định giới hạn LatLngBounds
                if (minLat == null || coordinates.latitude < minLat) minLat = coordinates.latitude;
                if (maxLat == null || coordinates.latitude > maxLat) maxLat = coordinates.latitude;
                if (minLng == null || coordinates.longitude < minLng) minLng = coordinates.longitude;
                if (maxLng == null || coordinates.longitude > maxLng) maxLng = coordinates.longitude;
                ...
                if (i == 0) {
                    ...
                    icon = BitmapDescriptor.fromBytes(startMarkerIcon); // Marker cuối cùng
                } else {
            ...
            if (minLat != null && minLng != null && maxLat != null && maxLng != null) {
                ...
                Future.delayed(Duration(milliseconds: 500), () async {
                    GoogleMapController? controller = await _customInfoWindowController.googleMapController;
                    if (controller != null) {
    ...
    void _showInfoWindow(GpsLogInfo gpsLogInfo) {
        ...
        bool isInside = _support.isPointInPolygon(coordinates, Constants.COORDINATES_LINE);
    ...
    Future<void> _sendAndReceiveDataFromListGpsScreen(List<GpsLogInfo> listGpsLog) async {
        final result = await Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => ListGpsScreen(listGpsLog: listGpsLog,)),
        );
